#!/usr/bin/env python3

import json
import math
import os
from pathlib import Path
from urllib import parse, request

import joblib
import numpy as np
import pandas as pd

from sklearn.compose import ColumnTransformer
from sklearn.ensemble import GradientBoostingRegressor
from sklearn.metrics import mean_absolute_error, mean_squared_error
from sklearn.pipeline import Pipeline
from sklearn.preprocessing import OneHotEncoder, StandardScaler


SUPABASE_URL = os.environ["SUPABASE_URL"].rstrip("/")
SUPABASE_KEY = os.environ["SUPABASE_SECRET_KEY"]
CLUB = "Lech Poznań"

PREDICT_OPPONENT = os.getenv("PREDICT_OPPONENT", "Raków Częstochowa")
PREDICT_MATCH_DATE = os.getenv(
    "PREDICT_MATCH_DATE",
    "2026-09-06T17:30:00+02:00",
)

ART = Path("model_artifacts_v03")
ART.mkdir(exist_ok=True)


def get_rows(table, params):
    q = parse.urlencode(params, doseq=True, safe=",.*()")
    url = f"{SUPABASE_URL}/rest/v1/{table}?{q}"
    req = request.Request(
        url,
        headers={
            "apikey": SUPABASE_KEY,
            "Authorization": f"Bearer {SUPABASE_KEY}",
        },
        method="GET",
    )
    with request.urlopen(req, timeout=60) as r:
        return json.loads(r.read().decode("utf-8"))


def load_data():
    h = pd.DataFrame(
        get_rows(
            "historical_matches",
            {
                "select": (
                    "id,club,opponent,attendance,season,"
                    "match_date,restricted_capacity"
                ),
                "club": f"eq.{CLUB}",
                "restricted_capacity": "eq.false",
                "order": "match_date.asc",
                "limit": "1000",
            },
        )
    )

    c = pd.DataFrame(
        get_rows(
            "historical_match_context",
            {
                "select": "*",
                "order": "match_date.asc",
                "limit": "1000",
            },
        )
    )

    if h.empty or c.empty:
        raise RuntimeError("Missing historical attendance or context.")

    h["match_date"] = pd.to_datetime(h["match_date"], utc=True)
    c["match_date"] = pd.to_datetime(c["match_date"], utc=True)

    df = h.merge(
        c,
        left_on="id",
        right_on="historical_match_id",
        how="inner",
        suffixes=("", "_ctx"),
    ).sort_values("match_date")

    return df.reset_index(drop=True)


def features(df):
    df = df.copy()

    local = df["match_date"].dt.tz_convert("Europe/Warsaw")
    df["month"] = local.dt.month
    df["weekday"] = local.dt.weekday
    df["kickoff_minutes"] = local.dt.hour * 60 + local.dt.minute

    prior = df["attendance"].shift(1)
    df["prev_attendance"] = prior
    df["rolling_3"] = prior.rolling(3, min_periods=1).mean()
    df["rolling_5"] = prior.rolling(5, min_periods=1).mean()
    df["rolling_10"] = prior.rolling(10, min_periods=1).mean()

    df["global_prev_mean"] = prior.expanding().mean()

    df["opponent_prev_mean"] = (
        df.groupby("opponent")["attendance"]
        .transform(lambda s: s.shift(1).expanding().mean())
        .fillna(df["global_prev_mean"])
    )

    df["opponent_draw_ratio"] = (
        df["opponent_prev_mean"]
        / df["global_prev_mean"].replace(0, np.nan)
    ).clip(0.50, 1.80)

    df["opponent_scaled"] = (
        df["rolling_5"] * df["opponent_draw_ratio"]
    )

    df["sport_form_gap"] = (
        df["lech_last5_points"]
        - df["opponent_last5_points"]
    )

    df["sport_gd_form_gap"] = (
        df["lech_last5_goal_diff"]
        - df["opponent_last5_goal_diff"]
    )

    df["ppg_gap"] = (
        df["lech_ppg_before"]
        - df["opponent_ppg_before"]
    )

    df["target_residual"] = (
        df["attendance"] - df["rolling_5"]
    )

    return df.dropna(
        subset=[
            "rolling_5",
            "opponent_draw_ratio",
            "lech_position_before",
            "opponent_position_before",
        ]
    ).reset_index(drop=True)


CATEGORICAL = ["opponent"]

NUMERIC = [
    "month",
    "weekday",
    "kickoff_minutes",
    "rolling_3",
    "rolling_5",
    "rolling_10",
    "opponent_draw_ratio",

    "round_no",
    "matches_remaining",
    "season_progress",

    "lech_position_before",
    "opponent_position_before",
    "position_gap",

    "lech_points_before",
    "opponent_points_before",
    "points_gap",
    "points_to_leader",

    "lech_ppg_before",
    "opponent_ppg_before",
    "ppg_gap",

    "lech_last5_points",
    "opponent_last5_points",
    "sport_form_gap",

    "lech_last5_goal_diff",
    "opponent_last5_goal_diff",
    "sport_gd_form_gap",
]

FEATURES = CATEGORICAL + NUMERIC


def build_model():
    pre = ColumnTransformer(
        [
            (
                "cat",
                OneHotEncoder(
                    handle_unknown="ignore",
                    sparse_output=False,
                ),
                CATEGORICAL,
            ),
            (
                "num",
                StandardScaler(),
                NUMERIC,
            ),
        ],
        remainder="drop",
    )

    reg = GradientBoostingRegressor(
        random_state=42,
        n_estimators=160,
        learning_rate=0.025,
        max_depth=2,
        min_samples_leaf=7,
        loss="huber",
    )

    return Pipeline(
        [
            ("pre", pre),
            ("model", reg),
        ]
    )


def metrics(y, p):
    y = np.asarray(y, float)
    p = np.asarray(p, float)
    e = p - y

    return {
        "mae": round(float(mean_absolute_error(y, p)), 1),
        "rmse": round(
            float(math.sqrt(mean_squared_error(y, p))),
            1,
        ),
        "mape_pct": round(
            float(np.mean(np.abs(e / y)) * 100),
            2,
        ),
        "bias": round(float(np.mean(e)), 1),
    }


def backtest(df):
    test_seasons = [
        x
        for x in ["2024/2025", "2025/2026", "2026/2027"]
        if x in set(df["season"])
    ]

    out = []

    for season in test_seasons:
        test = df[df["season"] == season].copy()
        train = df[
            df["match_date"] < test["match_date"].min()
        ].copy()

        if len(train) < 70 or test.empty:
            continue

        model = build_model()
        model.fit(
            train[FEATURES],
            train["target_residual"],
        )

        adjustment = model.predict(test[FEATURES])
        forecast = test["rolling_5"].to_numpy() + adjustment

        for pos, (_, r) in enumerate(test.iterrows()):
            out.append(
                {
                    "season": season,
                    "match_date": r["match_date"],
                    "opponent": r["opponent"],
                    "actual": float(r["attendance"]),
                    "rolling_5": float(r["rolling_5"]),
                    "opponent_scaled": float(r["opponent_scaled"]),
                    "model_v03": float(forecast[pos]),
                    "lech_position_before": int(r["lech_position_before"]),
                    "opponent_position_before": int(r["opponent_position_before"]),
                    "points_to_leader": int(r["points_to_leader"]),
                    "matches_remaining": int(r["matches_remaining"]),
                }
            )

    pred = pd.DataFrame(out)
    if pred.empty:
        raise RuntimeError("No v0.3 backtest predictions.")

    report = {
        col: metrics(pred["actual"], pred[col])
        for col in ["rolling_5", "opponent_scaled", "model_v03"]
    }

    per_season = {}
    for season, g in pred.groupby("season"):
        per_season[season] = {
            col: metrics(g["actual"], g[col])
            for col in ["rolling_5", "opponent_scaled", "model_v03"]
        }

    pred["model_error"] = pred["model_v03"] - pred["actual"]
    pred["model_abs_error"] = pred["model_error"].abs()

    top = pred.sort_values(
        "model_abs_error",
        ascending=False,
    ).head(10)

    return pred, report, per_season, top


def main():
    raw = load_data()
    df = features(raw)

    print(f"Context-enriched rows: {len(raw)}")
    print(f"Model rows: {len(df)}")

    pred, report, per_season, top = backtest(df)

    print("")
    print("MODEL A v0.3 — SPORTING CONTEXT BACKTEST")
    print("=" * 90)
    for name, m in report.items():
        print(
            f"{name:20}"
            f" MAE={m['mae']:8.1f}"
            f" RMSE={m['rmse']:8.1f}"
            f" MAPE={m['mape_pct']:6.2f}%"
            f" BIAS={m['bias']:8.1f}"
        )
    print("=" * 90)

    print("")
    print("PER-SEASON MAE")
    for season, vals in per_season.items():
        print(
            f"{season}: "
            f"rolling5={vals['rolling_5']['mae']:.1f}, "
            f"opponent_scaled={vals['opponent_scaled']['mae']:.1f}, "
            f"model_v03={vals['model_v03']['mae']:.1f}"
        )

    print("")
    print("TOP 10 ERRORS")
    print(top.to_string(index=False))

    pred.to_csv(
        ART / "backtest_predictions_v03.csv",
        index=False,
    )
    top.to_csv(
        ART / "top_10_errors_v03.csv",
        index=False,
    )

    report_payload = {
        "club": CLUB,
        "context_rows": int(len(raw)),
        "model_rows": int(len(df)),
        "backtest": report,
        "per_season": per_season,
        "note": (
            "v0.3 adds objectively reconstructed pre-match "
            "league-table and form context. No live ticket data."
        ),
    }

    (
        ART / "model_report_v03.json"
    ).write_text(
        json.dumps(
            report_payload,
            ensure_ascii=False,
            indent=2,
        ),
        encoding="utf-8",
    )

    final = build_model()
    final.fit(
        df[FEATURES],
        df["target_residual"],
    )
    joblib.dump(
        final,
        ART / "attendance_model_v03.joblib",
    )

    print("")
    print("SUCCESS")


if __name__ == "__main__":
    main()
